...
description = libs.versions.moduleDescriptionEventLogDuckdb.get()
...
dependencies {
  ...
  api(project(":atomic-event-log:iceberg"))
...
mavenPublishing {
  ...
  signAllPublications()
  coordinates(group.toString(), libs.versions.artifactEventLogDuckdb.get(), version.toString())

  pom {
    name.set(libs.versions.artifactEventLogDuckdb.get())
    description.set(libs.versions.pomDescriptionEventLogDuckdb.get())
