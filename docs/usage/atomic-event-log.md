...
This guide explains how `0.1.3` should be used to compose an event-log collector server.
...
ship a concrete `EventLogParquetFileRepository` implementation in `0.1.3`.
...
## 11. Recommended Default for `0.1.3`
...
- `EventLogParquetFileRepository` is still a host seam in `0.1.3`.
